...
const btnMayorPrecio = document.getElementById('mayorPrecio')
const btnMenorPrecio = document.getElementById('menorPrecio')
...
const cafeInfo = async () => {
  ...
  const orderCafe= products.sort((a,b)=>(a.price-b.price))
  const disponible = orderCafe.filter(cafe => cafe.available!== false)
  console.log(disponible);
  const noDisponible = orderCafe.filter(cafe=> cafe.available !==true)
  console.log(noDisponible);

  const unionCafes= disponible.concat(noDisponible);
  
  console.log(unionCafes);

  unionCafes.forEach((cafe, indice) => {
    if (indice >= 0 && indice <= 9) {
      // orderCafe.sort((a,b)=>b.price-a.price)
      ...
      if (cafe.available=== false) {
        ...
        btnBuy.style='width:70px;cursor:no-drop'
        divCard.style='gap:1em; opacity: 0.6;cursor:no-drop;'
      }
